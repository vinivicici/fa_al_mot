...
**가격 스케일 분석**:
- **H&M**: 평균 0.032, 범위 0.0004~0.507 (스케일링됨)
- **Fashion**: 평균 1,615, 범위 0~28,950 (원본 가격)

**가격 통일 방법**:
1. **H&M 가격 정규화**: 스케일링된 값 × 678.5
2. **Fashion 가격 정규화**: 원본 가격 × 0.011
3. **최종 통일**: 두 데이터셋 모두 정규화된 가격으로 통일

**예시**:
- H&M: 0.032 × 678.5 = 21.71
- Fashion: 1,615 × 0.011 = 17.77
...
### 도전과제
1. **가격 통일**: H&M과 Fashion의 다른 가격 스케일을 정규화 계수로 통일
...
2. **가격 통일**: H&M × 678.5, Fashion × 0.011로 정규화
